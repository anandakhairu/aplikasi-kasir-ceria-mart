...
def update_cart_display():
    ...
    header.pack(fill='x')
    tk.Label(header, text="Item", width=20, anchor='w', bg='#d0d0d0').pack(side='left')
    tk.Label(header, text="Price", width=10, anchor='center', bg='#d0d0d0').pack(side='left')
    tk.Label(header, text="Qty", width=12, anchor='center', bg='#d0d0d0').pack(side='left')
    tk.Label(header, text="Total", width=10, anchor='e', bg='#d0d0d0').pack(side='left')
...
def process_payment():
    ...
    receipt += "=" * 30 + "\n"
    receipt += f"Tanggal :  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    receipt += f"Invoice   :  INV/{datetime.now().strftime('%Y%m%d')}/{random.randint(1000,9999)}\n"
...
def change_qty(item_data, delta):
    item_data['qty'] += delta
    if item_data['qty'] <= 0:
        cart.remove(item_data)
    else:
        item_data['total'] = item_data['qty'] * item_data['price']
    update_cart_display()

def remove_item(item_data):
    if item_data in cart:
        cart.remove(item_data)
        update_cart_display()
